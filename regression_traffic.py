# File: regression/regression_traffic.py
import numpy as np
import pandas as pd
import os, sys
...
def load_traffic_data(file_path = 'data.mat'):
    
    data = sio.loadmat(file_path, squeeze_me=True)
    X_train_full = data["tra_X_tr"]
    X_test_full = data["tra_X_te"]
    y_train_full = data['tra_Y_tr']
    y_test_full = data['tra_Y_te']
   
    n_samples = 1261
    n_features = 48
    n_tests = 840
    location_id = 7 # max = 35
    
    X_train = np.zeros((n_samples,n_features))
    X_test =np.zeros((n_tests, n_features))
    y_train = y_train_full[location_id]
    y_test = y_test_full[location_id]
    
    for i in range(n_samples):
        X_train[i,:] = X_train_full[i].todense()[location_id,:]
    for i in range(n_tests):
        X_test[i,:] = X_test_full[i].todense()[location_id,:]
    
    df_xtrain = pd.DataFrame(X_train)
    df_xtrain = df_xtrain.loc[:, df_xtrain.nunique() > 1]#quchu 
    X_train = df_xtrain.values.astype(float)
    
    df_xtest = pd.DataFrame(X_test)
    df_xtest = df_xtest.loc[:, df_xtest.nunique() > 1]#quchu 
    X_test = df_xtest.values.astype(float)

    return X_train, y_train, X_test, y_test

if __name__ == "__main__":
    ########## working directory #########
    file_path="/mnt/f/pytorch/feature selection/datasets/traffic/traffic_dataset.mat"  
